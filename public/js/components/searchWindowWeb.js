...
function createQueryResultModalWeb() {
    const modalId = "queryResultModalWeb";
...
function showQueryResultModalWeb(apiUrl, datasetJson, query, parentid) {
    // Pagination State
    let currentPage = 1;
    ...
    const modal = createQueryResultModalWeb();
    ...
    async function fetchAndRenderData() {
        try {
            ...
            const response = await callApi(apiUrl, "GET"); // callApi should return the fetch Response object
            const data = await response.json();
            allData = data.data            // Initially, all data is unfiltered
            filteredData = [...(allData || [])];
            // Render the first page of data
            renderPaginatedGridWeb(filteredData, datasetJson.map((d) => d.fieldName));
            updatePaginationControlsWeb();
        } catch (error) {
    ...
    function renderPaginatedGridWeb(data, fields) {
    ...
    function updatePaginationControlsWeb() {
    ...
    nextPageBtn.addEventListener("click", () => {
        if (currentPage < Math.ceil(filteredData.length / pageSize)) {
            currentPage++;
            renderPaginatedGridWeb
                (filteredData, datasetJson.map((d) => d.fieldName));
            updatePaginationControlsWeb();
        }
    });
...
async function fetchQueryDataWeb(DBName, query) {
    try {
